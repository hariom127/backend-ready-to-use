...
exportFuns.downloadMediaByUrl = (mediaUrl, fileDestPath, callback) => {
  request(mediaUrl)
    .pipe(fs.createWriteStream(fileDestPath))
    .on("close", function () {
...
exportFuns.getMediaTypeByMimeType = (mimeType) => {
  var image_type_mimeTypes = [
    "image/jpeg",
    "image/jpg",
    "image/png",
    "image/gif",
  ];
  var video_type_mimeTypes = [
    "video/mp4",
    "video/3gp",
    "video/wmv",
    "video/webm",
...
exportFuns.isFileTypePdf = (mimeType) => {
...
exportFuns.generateXlsxFileByJson = (
  headerOpts = [],
  rowsData = [],
  filePath = ""
) => {
